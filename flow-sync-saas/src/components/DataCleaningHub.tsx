...
export function DataCleaningHub({
  ...
  const [delimiterInfo, setDelimiterInfo] = useState<{ delimiter: string; name: string }>({
    delimiter: ",",
    name: "Comma",
  });
  ...
  const [cleaningLogs, setCleaningLogs] = useState<
    Array<{ rowIdx: number; column: string; action: string; previous: string; current: string }>
  >([]);
  ...
  const handleDrop = (e: React.DragEvent) => {
    ...
    if (
      file &&
      (file.name.endsWith(".csv") || file.name.endsWith(".tsv") || file.name.endsWith(".txt"))
    ) {
      processFile(file);
    } else {
  ...
  const runStandardCleaning = () => {
    ...
    toast.success(
      `Standard auto-cleaning completed! Resolved ${result.logs.length} formatting issues.`,
    );
  };
  ...
  const runCustomInstructions = () => {
    ...
    setAiTerminalLogs((prev) => [
      ...prev,
      `[AI CONSULT] Interpreting instruction prompt: "${customAiPrompt}"...`,
    ]);
    ...
    setTimeout(() => {
      ...
      setAiTerminalLogs((prev) => [
        ...prev,
        ...result.aiLogs,
        `[AI CONSULT] Cleaning execution complete.`,
      ]);
      setCustomAiPrompt("");
  ...
  const handleImportToScheduler = () => {
    ...
    try {
      ...
      const standardizedRows = cleanedRows.map((row) => {
        ...
        std["Set up Time (Not related to any qty)"] = columnMapping.setupTime
          ? row[columnMapping.setupTime] || "0"
          : "0";
        std["Process time (related to qty)"] = columnMapping.processTime
          ? row[columnMapping.processTime] || "0"
          : "0";
        std["Base-Qty each process"] = columnMapping.baseQty
          ? row[columnMapping.baseQty] || "1"
          : "1";
      ...
      toast.success(
        `Dataset successfully committed! Loaded ${newMachines.length} custom resources under Group "${customGroupName}".`,
      );
      onSaved();
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-border/80 pb-5">
        <div className="space-y-1">
          ...
          <p className="text-sm text-muted-foreground max-w-2xl">
            Analyze, align delimiters, map custom headers (Assembly / Production lines), heal
            schedules with AI instructions, and preview analytics charts.
          </p>
        </div>
        <div className="flex gap-2.5">
          <Button
            variant="outline"
            size="sm"
            onClick={handleResetWorkspaceCSV}
      ...
      <Tabs value={mainPageTab} onValueChange={setMainPageTab} className="w-full">
        <TabsList className="bg-muted/80 border border-border/60 h-10 p-0.5 grid grid-cols-2 max-w-[400px] mb-4">
          <TabsTrigger
            value="workspace"
            className="text-xs data-[state=active]:bg-background data-[state=active]:text-foreground transition-all gap-1.5"
          >
            ...
            <div className="lg:col-span-5 space-y-6">
                ...
                <CardHeader className="pb-3 border-b border-border/50 flex flex-row items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="text-muted-foreground">
                      Drag and drop or select your CSV/TSV production files.
                    </CardDescription>
                  </div>
                  <Button
                    variant="ghost"
                ...
                <CardContent className="pt-4 space-y-4">
                  ...
                  >
                    ...
                    <p className="text-[10px] text-muted-foreground text-center mt-1">
                      Accepts CSV, TSV (Tab separated), Semicolon separated
                    </p>
                        ...
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex items-center gap-1.5">
                              <span className="text-[10px] font-extrabold text-primary font-mono bg-primary/10 px-1 py-0.5 rounded border border-primary/20">
                                {sample.icon}
                              </span>
                              <span className="text-[11px] font-bold text-foreground group-hover:text-primary transition-colors line-clamp-1">
                                {sample.name}
                              </span>
                            </div>
                            <Button
                          ...
                          <span className="text-[9px] text-muted-foreground line-clamp-1 mt-1">
                            {sample.industry}
                          </span>
                        </div>
                      ))}
                ...
                <Card className="border border-primary/30 shadow-md bg-gradient-to-b from-primary/5 to-transparent text-card-foreground animate-in fade-in duration-200">
                  <CardHeader className="pb-2 border-b border-primary/10">
                    ...
                    <CardDescription className="text-[11px]">
                      Our smart column mapping engine auto-detects English, German, and custom ERP
                      header names.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="pt-3 space-y-3">
                    <div className="space-y-2">
                      {EXPECTED_COLUMNS_SPEC.map((col) => (
                        <div
                          key={col.field}
                          className="p-2 bg-background rounded-md border border-border/50 text-[11px] space-y-1"
                        >
                          <div className="flex items-center justify-between">
                            <span className="font-bold text-foreground flex items-center gap-1">
                              {col.label}
                              {col.required ? (
                                <span className="text-[9px] bg-red-500/10 text-red-500 px-1 rounded font-mono">
                                  REQUIRED
                                </span>
                              ) : (
                                <span className="text-[9px] bg-muted text-muted-foreground px-1 rounded font-mono">
                                  OPTIONAL
                                </span>
                              )}
                            </span>
                          ...
                          <div className="flex flex-wrap items-center gap-1 pt-0.5">
                            ...
                            {col.aliases.slice(0, 4).map((alias) => (
                              <span
                                key={alias}
                                className="text-[9px] font-mono bg-muted/80 px-1 py-0.2 rounded border border-border/40"
                              >
                                {alias}
                              </span>
                            ))}
                    ...
                    <div className="p-2.5 bg-muted/40 rounded-lg border border-border/50 text-[10px] text-muted-foreground space-y-1">
                      ...
                      <ul className="list-disc pl-3.5 space-y-0.5 text-[9.5px]">
                        <li>
                          Dates support formats like <code className="font-mono">DD-MM-YYYY</code>,{" "}
                          <code className="font-mono">YYYY-MM-DD</code>,{" "}
                          <code className="font-mono">01 July 2026</code>.
                        </li>
                        <li>
                          Delimiters support Comma (<code className="font-mono">,</code>), Semicolon
                          (<code className="font-mono">;</code>), Tab (
                          <code className="font-mono">\t</code>), and Pipe (
              ...
              {rawRows.length > 0 && (
                <Card className="border border-border/80 shadow-sm bg-card text-card-foreground">
                  <CardHeader className="pb-3 border-b border-border/50">
                    ...
                    <CardDescription className="text-muted-foreground">
                      Map CSV headers to required scheduler fields.
                    </CardDescription>
                  </CardHeader>
                  <CardContent className="pt-4 space-y-3.5">
                    ...
                    <div className="space-y-1 bg-muted/30 p-2.5 rounded-lg border border-border/60">
                      <Label
                        htmlFor="resource-type-selector"
                      ...
                      >
                        <option value="Assembly Line">
                          Assembly Line (e.g. Line Alpha, Line Beta)
                        </option>
                        <option value="Production Line">
                          Production Line (e.g. Line 1, Line 2)
                        </option>
                        <option value="Machine Group">
                          Machine Group (e.g. Group M1, Group M2)
                        </option>
                        <option value="Workstation">Workstation (e.g. Milling A, Drill B)</option>
                      </select>
                      <span className="text-[9px] text-muted-foreground italic block mt-1">
                        This sets the name of the main resource group.
                      </span>
                    </div>
              ...
              <Card className="border border-border/80 shadow-sm bg-card text-card-foreground">
                <CardHeader className="pb-3 border-b border-border/50">
                  ...
                  <CardDescription className="text-muted-foreground">
                    Logical and formatting scan results of the uploaded document.
                  </CardDescription>
                </CardHeader>
                  ...
                  {rawRows.length === 0 ? (
                    ...
                    <Tabs value={activeViewTab} onValueChange={setActiveViewTab} className="w-full">
                      <TabsList className="grid w-full grid-cols-3 bg-muted border border-border/40 h-9 p-0.5">
                        <TabsTrigger
                          value="issues"
                          className="text-[10px] data-[state=active]:bg-background data-[state=active]:text-foreground"
                        >
                      ...
                      <TabsContent
                        value="issues"
                        className="pt-3 max-h-[260px] overflow-y-auto space-y-2"
                      >
                        {validationIssues.length === 0 ? (
                          <div className="flex flex-col items-center justify-center py-6 text-center text-emerald-500 dark:text-emerald-400 gap-1.5">
                            ...
                            <span className="text-[10px] text-muted-foreground">
                              Standard scheduler fields are clean and formatted.
                            </span>
                          </div>
                        ) : (
                              ...
                              <div className="space-y-0.5">
                                ...
                                {issue.column && (
                                  <div className="flex gap-2 text-[9px] opacity-70">
                                    <span>
                                      Col: <strong className="font-mono">{issue.column}</strong>
                                    </span>
                                    {issue.rowIdx >= 0 && (
                                      <span>
                                        Row: <strong>{issue.rowIdx + 1}</strong>
                                      </span>
                                    )}
                                  </div>
                                )}
                      ...
                      <TabsContent value="options" className="pt-3 space-y-3">
                        <div className="space-y-2.5 bg-muted/20 border border-border p-3 rounded-lg text-[11px]">
                          <div className="flex items-center space-x-2">
                            ...
                            <label
                              htmlFor="opt-trim"
                              className="font-medium cursor-pointer text-foreground leading-none"
                            >
                              Trim inner tab-spaces & carriage returns
                            </label>
                          </div>
                          <p className="text-[9px] text-muted-foreground pl-5">
                            Strips excessive spacing, carriage returns, and tabs in material
                            strings.
                          </p>
                          ...
                          <div className="flex items-center space-x-2 pt-1">
                            <Checkbox
                              ...
                              onCheckedChange={(checked) =>
                                setCleaningOptions((prev) => ({ ...prev, fixNegatives: !!checked }))
                              }
                            />
                            <label
                              htmlFor="opt-neg"
                          ...
                          <div className="flex items-center space-x-2 pt-1">
                            <Checkbox
                              ...
                              onCheckedChange={(checked) =>
                                setCleaningOptions((prev) => ({ ...prev, fillDefaults: !!checked }))
                              }
                            />
                            <label
                              htmlFor="opt-defaults"
                              className="font-medium cursor-pointer text-foreground leading-none"
                            >
                      ...
                      >
                        {cleaningLogs.length === 0 ? (
                          ...
                          cleaningLogs.map((log, idx) => (
                            <div
                              key={idx}
                              className="p-2 border border-border bg-card rounded text-[10px] font-mono space-y-1 shadow-sm"
                            >
                              <div className="flex justify-between border-b border-border/50 pb-0.5 font-bold">
                                <span className="text-foreground">
                                  Row {log.rowIdx + 1} &middot; {log.column}
                                </span>
                                <span className="text-[8px] text-emerald-600 bg-emerald-50 px-1 rounded border border-emerald-200/50">
                                  {log.action}
                                </span>
                              </div>
                              <div className="grid grid-cols-2 gap-1 text-[9px] text-muted-foreground mt-1">
                                <div>
                                  Before:{" "}
                                  <strong className="text-red-500 line-through font-normal">
                                    {log.previous || "BLANK"}
                                  </strong>
                                </div>
                                <div>
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <Card className="bg-gradient-to-br from-indigo-50/20 to-slate-50/20 dark:from-indigo-950/5 dark:to-zinc-900/5 border border-border shadow-md">
                <CardHeader className="pb-3 border-b border-border/40">
                  ...
                  <CardDescription className="text-muted-foreground">
                    Heal dataset discrepancies using contextual model recommendations or custom
                    commands.
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-4 space-y-4">
                  ...
                  {rawRows.length > 0 && aiSuggestions.length > 0 && (
                    <div className="space-y-2">
                      <span className="text-[11px] font-bold text-muted-foreground block">
                        AI Smart Heuristic Suggestions:
                      </span>
                      <div className="grid gap-2 sm:grid-cols-2 max-h-[170px] overflow-y-auto">
                        {aiSuggestions.map((sugg) => (
                          <div
                            key={sugg.id}
                            className="bg-card border border-indigo-100 dark:border-indigo-900 p-2.5 rounded-lg text-xs space-y-2 flex flex-col justify-between shadow-sm"
                          >
                            <div className="space-y-1">
                              <div className="flex items-center justify-between">
                  ...
                  <div className="space-y-2">
                    ...
                    >
                      ...
                      <span className="text-[9px] font-normal text-muted-foreground/80 italic">
                    ...
                    {rawRows.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        ...
                        <button
                          onClick={() =>
                            setCustomAiPrompt(`Assign Assy Line Alpha to all casing steps`)
                          }
                        ...
                        <button
                          onClick={() =>
                            setCustomAiPrompt(
                              `double quantity for order ${rawRows[0]?.[columnMapping.order] || "100501"}`,
                            )
                          }
                          className="text-[9px] bg-background hover:bg-muted border border-border/80 text-muted-foreground px-2 py-0.5 rounded cursor-pointer transition-colors"
                        >
                  ...
                  <div className="rounded-lg bg-slate-50 dark:bg-zinc-950 border border-border text-slate-700 dark:text-zinc-350 p-3 font-mono text-[10px] space-y-1 relative shadow-inner">
                    <div className="flex items-center justify-between border-b border-slate-200 dark:border-zinc-800/80 pb-1.5 mb-1.5 text-slate-500 font-bold text-[8px] uppercase tracking-wider">
                      <span className="flex items-center gap-1">
                        <Terminal className="h-3 w-3" /> AI Reasoning Console
                      </span>
                      ...
                      {aiTerminalLogs.map((log, idx) => (
                        <div key={idx} className="leading-relaxed">
                          {log.startsWith("[SYSTEM]") && (
                            <span className="text-sky-600 dark:text-sky-400">{log}</span>
                          )}
                          {log.startsWith("[CLEANER]") && (
                            <span className="text-amber-600 dark:text-amber-400">{log}</span>
                          )}
                          {(log.startsWith("[AI CO-PILOT]") || log.startsWith("[AI ENGINE]")) && (
                            <span className="text-indigo-600 dark:text-indigo-400">{log}</span>
                          )}
                          {log.startsWith("[AI CONSULT]") && (
                            <span className="text-pink-600 dark:text-pink-400">{log}</span>
                          )}
                          {log.startsWith("[AI HEAL]") && (
                            <span className="text-emerald-600 dark:text-emerald-400">{log}</span>
                          )}
                          {!log.startsWith("[") && (
                            <span className="text-slate-600 dark:text-zinc-400">{log}</span>
                          )}
                        </div>
                      ))}
              ...
              {rawRows.length > 0 && (
                <Card className="border border-border/80 shadow-sm overflow-hidden bg-card text-card-foreground">
                  <CardHeader className="pb-2 flex flex-row items-center justify-between border-b border-border/50 bg-slate-50/50 dark:bg-slate-950/20">
                    ...
                    <Tabs
                      value={activeGridTab}
                      onValueChange={setActiveGridTab}
                      className="bg-muted border border-border/55 p-0.5 rounded-md h-7 shrink-0"
                    >
                      <TabsList className="h-full p-0">
                        <TabsTrigger
                          value="cleaned"
                          className="text-[10px] h-full px-2.5 data-[state=active]:bg-background data-[state=active]:text-foreground"
                        >
                          Cleaned Output
                        </TabsTrigger>
                        <TabsTrigger
                      ...
                      <Table className="text-[11px] font-mono select-none">
                        <TableHeader className="bg-slate-50/80 dark:bg-slate-900 border-b border-border sticky top-0 z-10">
                          <TableRow className="border-b border-border hover:bg-transparent">
                            <TableHead className="font-bold text-muted-foreground border-r border-border w-[45px] text-center">
                              Row
                            </TableHead>
                            ...
                            <TableHead className="font-bold text-foreground border-r border-border">
                              {columnMapping.material}
                            </TableHead>
                            <TableHead className="font-bold text-foreground border-r border-border">
                              {columnMapping.processId}
                            </TableHead>
                            <TableHead className="font-bold text-foreground border-r border-border">
                              {columnMapping.qty}
                            </TableHead>
                            <TableHead className="font-bold text-foreground border-r border-border">
                              {columnMapping.sopStartDate}
                            </TableHead>
                            <TableHead className="font-bold text-primary border-r border-border">
                              {columnMapping.machine}
                            </TableHead>
                            {columnMapping.setupTime && (
                              <TableHead className="font-bold text-muted-foreground border-r border-border">
                                Setup
                              </TableHead>
                            )}
                            {columnMapping.processTime && (
                        ...
                        <TableBody>
                          {(activeGridTab === "cleaned" ? cleanedRows : rawRows).map((row, idx) => {
                            const isModified =
                              activeGridTab === "cleaned" &&
                              JSON.stringify(row) !== JSON.stringify(rawRows[idx]);
                            ...
                            return (
                              <TableRow
                                ...
                                className={`hover:bg-muted/5 border-b border-border/30 ${
                                  isModified
                                    ? "bg-emerald-500/5 hover:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 font-semibold"
                                    : ""
                                }`}
                              >
                                <TableCell className="text-center font-bold text-muted-foreground border-r border-border/30 w-[45px]">
                                  {idx + 1}
                                </TableCell>
                                <TableCell className="border-r border-border/30 font-semibold">
                                  {row[columnMapping.order]}
                                </TableCell>
                                <TableCell
                                  className="border-r border-border/30 truncate max-w-[130px] font-sans"
                                  title={row[columnMapping.material]}
                                >
                                  {row[columnMapping.material]}
                                </TableCell>
                                <TableCell className="border-r border-border/30 text-center">
                                  {row[columnMapping.processId]}
                                </TableCell>
                                <TableCell className="border-r border-border/30 text-right">
                                  {row[columnMapping.qty]}
                                </TableCell>
                                <TableCell className="border-r border-border/30 text-center font-sans">
                                  {row[columnMapping.sopStartDate]}
                                </TableCell>
                                <TableCell className="border-r border-border/30 text-center font-sans font-bold text-primary">
                                  {row[columnMapping.machine]}
                                </TableCell>
                                {columnMapping.setupTime && (
                                  <TableCell className="border-r border-border/30 text-right">
                                    {row[columnMapping.setupTime]}
                                  </TableCell>
                                )}
                                {columnMapping.processTime && (
                                  <TableCell className="text-right">
                                    {row[columnMapping.processTime]}
                                  </TableCell>
                                )}
                              </TableRow>
                            );
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <Card className="border border-border/80 shadow-sm bg-card text-card-foreground">
                <CardHeader className="pb-3 border-b border-border/50">
                  ...
                  <CardDescription className="text-muted-foreground">
                    Summed setup + processing times grouped per active resource.
                  </CardDescription>
                </CardHeader>
              ...
              <Card className="border border-border/80 shadow-sm bg-card text-card-foreground">
                <CardHeader className="pb-3 border-b border-border/50">
                  ...
                  <CardDescription className="text-muted-foreground">
                    Daily start volume and counts of orders across timeline.
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-6">
                  <div className="h-[280px] w-full text-xs">
              ...
              <Card className="border border-border/80 shadow-sm bg-card text-card-foreground md:col-span-2">
                <CardHeader className="pb-3 border-b border-border/50">
                  ...
                  <CardDescription className="text-muted-foreground">
                    Ranking of the top 8 materials by target scheduling quantities.
                  </CardDescription>
                </CardHeader>
                <CardContent className="pt-6">
                  <div className="h-[280px] w-full text-xs">
