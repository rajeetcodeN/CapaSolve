...
export const Route = createFileRoute("/monthly")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Monthly capacity scheduler and post-optimization evaluation dashboard." },
    ],
  }),
...
function MonthlyPlannerPage() {
  ...
  const initialDate = useMemo(() => {
    const validStart = processes.find((p) => p.scheduledStart)?.scheduledStart || orders[0]?.sopStartDate;
    if (validStart) {
      try {
  ...
  const handleExportHTMLReport = (start: string, end: string) => {
    ...
    while (tempPtr <= endDate) {
      ...
      const cap = dailyCapacities?.[dStr] || { setter: globalSetterCapacity, process: globalOperatorCapacity };
    ...
    const rangeLabel = language === "de" ? `Zeitraum: ${start} bis ${end}` : `Date Range: ${start} to ${end}`;
    const statsLabel = language === "de" ? "Planungsstatistiken" : "Plan Summary Metrics";
    const chartLabel = language === "de" ? "Kapazitätsauslastung (Kalenderübersicht)" : "Capacity Workload Distribution Chart";
    const detailsLabel = language === "de" ? "Detaillierte Prozessschritte" : "Detailed Scheduled Processes";
      ...
      <body>
        ...
        <div class="calendar-grid">
          ${daysList.map((d) => {
            ...
            const barColor = d.resourceUtilPct > 90 ? "#ef4444" : d.resourceUtilPct > 50 ? "#f59e0b" : "#10b981";
            return `
              <div class="day-cell">
        ...
        <table>
          ...
          <tbody>
            ${rangeComparisons.map((c) => {
              const badgeClass = c.status === "PUSHED_FORWARD" ? "badge-pushed" : c.status === "ON_TIME" ? "badge-ontime" : "badge-delayed";
              const formattedStart = c.scheduledStart ? c.scheduledStart.toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" }) : "—";
              const formattedEnd = c.scheduledEnd ? c.scheduledEnd.toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" }) : "—";
              return `
                <tr>
  ...
  const handleToggleHolidayDirectly = (dayStr: string, isHoliday: boolean, dayNum: number) => {
    ...
    toast.success(
      targetIsHoliday
        ? (language === "de" ? `Tag ${dayNum} als Feiertag markiert` : `Day ${dayNum} marked as Holiday`)
        : (language === "de" ? `Feiertag für Tag ${dayNum} aufgehoben` : `Holiday removed for Day ${dayNum}`)
    );
  };
      ...
      if (status === "DELAYED" && plannedStart && scheduledStart) {
        ...
        if (!delayReason) {
          ...
          const blockingProcess = processes.find((op) => {
            if (op.id === p.id || op.machineId !== targetMachineId || !op.scheduledStart || !op.scheduledEnd) return false;
  ...
  const calendarDays = useMemo(() => {
    ...
    for (let i = 0; i < offset; i++) {
      list.push({ dayStr: "", dayNum: 0, isPadding: true, totalHours: 0, pushedCount: 0, processesCount: 0, isHoliday: false, resourceUtilPct: 0, setterCap: 100, operatorCap: 200 });
    }
    ...
    for (let i = 1; i <= totalDays; i++) {
      ...
      const pushedCount = comparisons.filter(
        (c) => c.scheduledStart && formatDateStr(c.scheduledStart) === dStr && c.status === "PUSHED_FORWARD"
      ).length;
    ...
    return list;
  }, [currentYear, currentMonth, slots, comparisons, dailyCapacities, globalSetterCapacity, globalOperatorCapacity]);
  ...
  const filteredComparisons = useMemo(() => {
    return comparisons.filter((c) => {
      ...
      const matchesMachine =
        machineFilter === "ALL" ||
        c.scheduledMachineId === machineFilter;
    ...
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="border-b border-border/60 pb-5">
        ...
        <p className="text-muted-foreground text-sm">
          {language === "de"
            ? "Analysieren Sie die monatliche Auslastung und vergleichen Sie den ursprünglichen SOP-Plan mit den durchgeführten Optimierungen."
            : "Review monthly workload distributions, identify pushed-forward scheduling opportunities, and track post-optimization overrides."}
        </p>
      </div>
          ...
          <div className="flex flex-row items-center justify-between flex-wrap gap-4">
            <div>
              <CardTitle className="text-sm font-bold flex items-center gap-2 text-foreground">
                ...
                {language === "de" ? "Live-Optimierungsstrategien" : "Live Optimization Strategy Dashboard"}
              </CardTitle>
              <CardDescription>
                {language === "de"
            ...
            <div className="flex gap-2">
              ...
              >
                {language === "de" ? "Alle deaktivieren (Nicht Optimiert)" : "Disable All (Non-Optimized)"}
              </Button>
              <Button 
                variant="default" 
                size="sm" 
              ...
              >
                {language === "de" ? "Alle aktivieren (Voll Optimiert)" : "Enable All (Fully Optimized)"}
              </Button>
            </div>
          </div>
        </CardHeader>
          ...
          <div className="grid gap-6 md:grid-cols-3">
            ...
            <div className={cn(
              ...
              <div className="mt-2 text-[10px] font-mono flex items-center gap-1">
                <span className={allowProcessOverlap ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-muted-foreground/60"}>
                  {allowProcessOverlap ? "✓ Active (Optimizing Capacity)" : "✗ Inactive (Strict Sequential)"}
                </span>
              </div>
            </div>
            ...
            <div className={cn(
              ...
              <div className="space-y-1">
                ...
                {allowSopOverride && (
                  <div className="mt-3 space-y-1 bg-slate-500/[0.03] dark:bg-slate-900/40 p-2 rounded border border-border/40 animate-in fade-in duration-200">
                    <Label htmlFor="max-prepone-monthly" className="text-[10px] font-bold text-muted-foreground uppercase">
                    ...
                    >
                      <SelectTrigger id="max-prepone-monthly" className="h-7 text-[11px] bg-background">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
              ...
              <div className="mt-2 text-[10px] font-mono flex items-center gap-1">
                <span className={allowSopOverride ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-muted-foreground/60"}>
                  {allowSopOverride ? "✓ Active (Pulling Orders Forward)" : "✗ Inactive (Respect SOP Date)"}
                </span>
              </div>
            </div>
            ...
            <div className={cn(
              ...
              <div className="space-y-1">
                <div className="flex items-center justify-between">
                  <span className="font-bold text-xs text-foreground">
                    {language === "de" ? "Max. Auslastung (Gruppenwechsel)" : "Max Utilize (Shift in Group)"}
                  </span>
                  <input
                    type="checkbox"
                ...
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  {language === "de"
                    ? "Erlaubt das Verschieben von Prozessen auf andere freie Maschinen derselben Gruppe."
                    : "Allows shifting processes to alternative workstations within the same machine group."}
                </p>
              </div>
              <div className="mt-2 text-[10px] font-mono flex items-center gap-1">
                <span className={maxUtilizeResources ? "text-emerald-600 dark:text-emerald-400 font-bold" : "text-muted-foreground/60"}>
                  {maxUtilizeResources ? "✓ Active (Workstation Swapping Enabled)" : "✗ Inactive (Original Workstation Only)"}
                </span>
              </div>
            </div>
          </div>
      ...
      <div className="grid gap-4 md:grid-cols-4">
        <Card className="border border-border/60 shadow-sm">
          ...
          <CardContent>
            <div className="text-2xl font-bold font-mono text-primary">{rollupStats.totalScheduled}</div>
            <p className="text-[10px] text-muted-foreground mt-0.5">
              {language === "de" ? "Gesamtzahl optimierter Arbeitsschritte" : "Total allocation processes"}
            </p>
          </CardContent>
        </Card>

        <Card className="border border-border/60 shadow-sm bg-emerald-500/[0.02] border-emerald-500/20">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-semibold text-emerald-600 dark:text-emerald-400 uppercase flex items-center gap-1">
          ...
          <CardContent>
            <div className="text-2xl font-bold font-mono text-emerald-600 dark:text-emerald-400">
              {rollupStats.pushedCount} <span className="text-xs font-medium font-sans">({rollupStats.pushedPct.toFixed(0)}%)</span>
            </div>
            <p className="text-[10px] text-muted-foreground mt-0.5">
              {language === "de" ? "Früher als geplant (SOP) beendet" : "Scheduled earlier than original SOP"}
            </p>
          </CardContent>
        </Card>

        <Card className="border border-border/60 shadow-sm bg-amber-500/[0.02] border-amber-500/20">
          <CardHeader className="pb-2">
          ...
          <CardContent>
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5">
              {language === "de" ? "Genau zum SOP-Termin eingeplant" : "Scheduled exactly on SOP release"}
            </p>
          </CardContent>
        </Card>

        <Card className="border border-border/60 shadow-sm bg-indigo-500/[0.02] border-indigo-500/20">
          <CardHeader className="pb-2">
            <CardTitle className="text-xs font-semibold text-indigo-600 dark:text-indigo-400 uppercase flex items-center gap-1">
          ...
          <CardContent>
            ...
            <p className="text-[10px] text-muted-foreground mt-0.5">
              {language === "de" ? "Zuweisung auf Alternativmaschinen" : "Rescheduled to alternative workstations"}
            </p>
          </CardContent>
        </Card>
      </div>
        ...
        <DialogContent className="sm:max-w-[425px] font-sans">
          ...
          <div className="grid gap-4 py-4 text-xs">
            ...
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="export-start-date" className="text-right font-bold text-muted-foreground">
            ...
            {/* End Date */}
            <div className="grid grid-cols-4 items-center gap-4">
              <Label htmlFor="export-end-date" className="text-right font-bold text-muted-foreground">
          ...
          <DialogFooter className="flex flex-col sm:flex-row gap-2 mt-2">
            <Button variant="outline" size="sm" onClick={() => setIsExportModalOpen(false)} className="w-full sm:w-auto">
          ...
          <CardHeader className="pb-3 border-b border-border/40 flex flex-row items-center justify-between flex-wrap gap-4">
            <div>
              <CardTitle className="text-sm font-bold flex items-center gap-2 text-foreground">
                ...
                {language === "de" ? "Monatliche Kapazitätsauslastung" : "Monthly Workload Distribution"}
              </CardTitle>
              <CardDescription>
                {language === "de"
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  toast.info(`Jumped to active data: ${monthNames[initialDate.month]} ${initialDate.year}`);
                }}
              >
                Active Data Month
              </Button>
          ...
          <CardContent className="pt-4 px-4 pb-4">
            ...
            <div className="grid grid-cols-7 gap-2 font-mono text-xs">
              {calendarDays.map((cd, idx) => {
                if (cd.isPadding) {
                  return <div key={idx} className="min-h-[110px] bg-slate-50/20 dark:bg-slate-900/10 rounded-md border border-dashed border-border/20" />;
                }
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex items-center gap-1">
                        {cd.isHoliday && (
                          <span className="text-[8px] bg-rose-500 text-rose-50 px-1 py-0.2 rounded font-sans uppercase font-bold scale-90">H</span>
                        )}
                        ...
                        <div className="opacity-0 group-hover:opacity-100 transition-opacity flex items-center gap-1">
                          <button
                            ...
                            className={cn(
                              ...
                              cd.isHoliday ? "text-rose-500 hover:text-rose-600 bg-rose-50 dark:bg-rose-950/20" : "hover:text-rose-500"
                            )}
                            title={cd.isHoliday ? (language === "de" ? "Feiertag entfernen" : "Remove Holiday") : (language === "de" ? "Als Feiertag markieren" : "Mark as Holiday")}
                          >
                      ...
                      <div className="space-y-1 my-1">
                        {dayProcsList.slice(0, 2).map((proc: any) => {
                          const startTimeStr = proc.scheduledStart
                            ? new Date(proc.scheduledStart).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                            : "";
                          const endTimeStr = proc.scheduledEnd
                            ? new Date(proc.scheduledEnd).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })
                            : "";
                          ...
                          return (
                            ...
                            >
                              <div className="truncate flex-1">
                                <span className="font-bold">{proc.orderId}-{proc.processId}</span>
                                <span className="text-[8.5px] opacity-80 block truncate">{startTimeStr} ({proc.scheduledMachineId})</span>
                              </div>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                              ...
                              >
                                {proc.isManual ? <Pin className="h-3 w-3 text-amber-500 fill-current" /> : <PinOff className="h-3 w-3 opacity-40 hover:opacity-100" />}
                              </button>
                            </div>
                          );
                        })}
                    ...
                    {(cd.processesCount > 0 || cd.setterCap !== globalSetterCapacity || cd.operatorCap !== globalOperatorCapacity) && !cd.isHoliday ? (
                      <div className="space-y-0.5 w-full mt-auto">
                        <div className="w-full h-1 rounded-full bg-slate-100 dark:bg-slate-800 overflow-hidden flex border border-border/10">
                          <div
                            ...
                            className={cn(
                              "h-full",
                              cd.resourceUtilPct > 90 ? "bg-red-500" : cd.resourceUtilPct > 50 ? "bg-amber-500" : "bg-emerald-500"
                            )}
                          />
                        </div>
                        <div className="flex justify-between items-center text-[8.5px] text-muted-foreground font-mono">
                          <span className="font-bold text-foreground">{cd.resourceUtilPct.toFixed(0)}% Util</span>
      ...
      <Dialog open={!!selectedDayDetails} onOpenChange={(open) => !open && setSelectedDayDetails(null)}>
        <DialogContent className="sm:max-w-xl font-sans">
          <DialogHeader>
            ...
            <DialogDescription className="text-xs">
              Review live scheduled start/end times, workstation assignments, and pin/unpin process steps.
            </DialogDescription>
          </DialogHeader>
          ...
          {selectedDayDetails && (
            <div className="space-y-3 py-2 text-xs max-h-[400px] overflow-y-auto">
              {selectedDayDetails.dayProcs.length === 0 ? (
                <p className="text-muted-foreground text-center py-4">No processes scheduled on this day.</p>
              ) : (
                    ...
                    >
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          <span className="font-extrabold font-mono text-primary text-xs">Order {proc.orderId} (Step {proc.processId})</span>
                          {proc.isManual && (
                            <span className="text-[9px] bg-amber-500 text-white font-bold px-1.5 py-0.2 rounded uppercase">PINNED</span>
                          )}
                        </div>
                        <p className="text-xs font-semibold">{proc.material} — {proc.processText}</p>
                        <div className="flex items-center gap-3 text-[11px] text-muted-foreground font-mono">
                          <span>Workstation: <strong className="text-foreground">{proc.scheduledMachineId}</strong></span>
                          <span>Time: <strong className="text-foreground">{startTimeStr} - {endTimeStr}</strong></span>
                        </div>
                      </div>
      ...
      <Card className="border border-border/80 shadow-md bg-card overflow-hidden">
        <CardHeader className="border-b border-border/50 pb-3 flex flex-row items-center justify-between flex-wrap gap-4">
          <div>
            <CardTitle className="text-base font-bold flex items-center gap-2 text-foreground">
              ...
              {language === "de" ? "Abgleich: Soll-Planung vs. Ist-Planung (Post-Optimiert)" : "SOP Planned vs. Scheduled Post-Optimized Comparison"}
            </CardTitle>
            <CardDescription>
              {language === "de"
        ...
        <div className="bg-muted/30 border-b border-border p-4 grid gap-4 grid-cols-1 sm:grid-cols-4">
          <div className="space-y-1">
            <Label htmlFor="search-cmp" className="text-xs font-semibold text-muted-foreground uppercase flex items-center gap-1.5">
          ...
          <div className="space-y-1">
            <Label htmlFor="filter-status" className="text-xs font-semibold text-muted-foreground uppercase flex items-center gap-1.5">
            ...
            <Select value={statusFilter} onValueChange={setStatusFilter}>
              ...
              <SelectContent>
                <SelectItem value="ALL">{language === "de" ? "Alle Planungszustände" : "All Statuses"}</SelectItem>
                <SelectItem value="PUSHED_FORWARD">{language === "de" ? "Vorgezogen" : "Pushed Forward"}</SelectItem>
                <SelectItem value="ON_TIME">{language === "de" ? "Pünktlich" : "On Time"}</SelectItem>
                <SelectItem value="DELAYED">{language === "de" ? "Verzögert" : "Delayed"}</SelectItem>
              </SelectContent>
            </Select>
          </div>

          <div className="space-y-1">
            <Label htmlFor="filter-machine-monthly" className="text-xs font-semibold text-muted-foreground uppercase flex items-center gap-1.5">
            ...
            <Select value={machineFilter} onValueChange={setMachineFilter}>
              ...
              <SelectContent>
                <SelectItem value="ALL">{language === "de" ? "Alle Maschinen" : "All Workstations"}</SelectItem>
                {machines.map((m) => (
                  <SelectItem key={m.id} value={m.id}>{getMachineDisplay(m.id)}</SelectItem>
                ))}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="space-y-1">
            <Label htmlFor="filter-strategy" className="text-xs font-semibold text-muted-foreground uppercase flex items-center gap-1.5">
            ...
            <Select value={strategyFilter} onValueChange={setStrategyFilter}>
              ...
              <SelectContent>
                <SelectItem value="ALL">{language === "de" ? "Alle Strategien" : "All Strategies"}</SelectItem>
                <SelectItem value="NON_OPTIMIZED">{language === "de" ? "Nicht optimiert" : "Non-Optimized"}</SelectItem>
                <SelectItem value="OPTIMIZED">{language === "de" ? "Optimiert" : "Optimized"}</SelectItem>
                <SelectItem value="SOP_OVERRIDE">{language === "de" ? "SOP Override (Vorgezogen)" : "SOP Override (Pushed Forward)"}</SelectItem>
                <SelectItem value="GROUP_SHIFT">{language === "de" ? "Group Shift (Alt. Maschine)" : "Group Shift (Alt. Workstation)"}</SelectItem>
              </SelectContent>
            </Select>
          </div>
        </div>
          ...
          <table className="w-full text-xs text-left border-collapse font-sans">
            <thead>
              <tr className="bg-muted/50 border-b border-border text-muted-foreground font-bold">
                ...
                <th className="px-4 py-3">{language === "de" ? "Material / Vorgangstext" : "Material & Work description"}</th>
                <th className="px-4 py-3 border-l border-border bg-slate-500/[0.02]">{language === "de" ? "Soll-Start (Planned SOP)" : "Soll-Start (Planned SOP)"}</th>
                <th className="px-4 py-3 bg-slate-500/[0.02]">{language === "de" ? "Soll-Maschine" : "Soll-Machine"}</th>
                <th className="px-4 py-3 border-l border-border bg-primary/[0.01]">{language === "de" ? "Optimiertes Ist-Start" : "Optimized Scheduled Start"}</th>
                <th className="px-4 py-3 bg-primary/[0.01]">{language === "de" ? "Optimierte Maschine" : "Optimized Machine"}</th>
                <th className="px-4 py-3 border-l border-border text-center">{language === "de" ? "Abweichung / Effekt" : "Deviation / Effect"}</th>
                <th className="px-4 py-3 border-l border-border">{language === "de" ? "Ursache für Verzögerung" : "Delay Reason"}</th>
                <th className="px-4 py-3 border-l border-border text-center">{language === "de" ? "Sichern" : "Pin/Lock"}</th>
              </tr>
            </thead>
            <tbody>
              {filteredComparisons.length > 0 ? (
                filteredComparisons.map((c) => (
                  <tr key={c.id} className="border-b border-border hover:bg-muted/5 transition-colors">
                    <td className="px-4 py-3 font-bold text-foreground font-mono">{c.orderId}</td>
                    <td className="px-4 py-3 font-mono font-medium text-muted-foreground">{c.processId}</td>
                    <td className="px-4 py-3 max-w-[280px] truncate">
                      <div className="font-semibold text-foreground truncate">{c.material}</div>
                      <div className="text-[10px] text-muted-foreground truncate">{c.processText}</div>
                    </td>
                    
                    {/* Planned SOP */}
                    <td className="px-4 py-3 font-mono border-l border-border bg-slate-500/[0.01] text-muted-foreground">
                      {c.plannedStart ? c.plannedStart.toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" }) : "—"}
                    </td>
                    ...
                    <td className="px-4 py-3 font-mono border-l border-border bg-primary/[0.005] font-semibold text-foreground">
                      {c.scheduledStart ? c.scheduledStart.toLocaleString(undefined, { dateStyle: "short", timeStyle: "short" }) : "Unscheduled"}
                    </td>
                    <td className={cn(
                      "px-4 py-3 font-mono font-bold bg-primary/[0.005]",
                      c.hasMachineShift ? "text-indigo-600 dark:text-indigo-400 bg-indigo-500/[0.04]" : "text-foreground"
                    )}>
                      {getMachineDisplay(c.scheduledMachineId)}
                      {c.hasMachineShift && (
                        <span className="text-[9px] block text-muted-foreground font-normal">
                          {language === "de" ? `(zuvor Maschine ${c.originalMachineId})` : `(was Machine ${c.originalMachineId})`}
                        </span>
                      )}
                    </td>
                    ...
                    <td className="px-4 py-3 border-l border-border text-center font-mono align-middle">
                      <div className="flex flex-col items-center justify-center gap-1">
                        <span className={cn(
                          ...
                          {c.status === "PUSHED_FORWARD" && (language === "de" ? "Vorgezogen" : "Pushed Forward")}
                        ...
                        {c.diffText && (
                          <span className="text-[10px] text-muted-foreground font-medium">{c.diffText}</span>
                        )}
                      </div>
                    </td>
                    <td className="px-4 py-3 border-l border-border text-muted-foreground font-medium max-w-[200px] truncate" title={c.delayReason || undefined}>
                      {c.delayReason ? (
                        <div className="flex items-center gap-1">
                        ...
                        c.isManual ? (
                          <Button
                            ...
                            onClick={() => {
                              ...
                              toast.success(language === "de" ? "Pin aufgehoben!" : "Schedule unpinned!");
                            }}
                          ...
                          <Button
                            ...
                            onClick={() => {
                              ...
                              toast.success(language === "de" ? "Planung gesichert!" : "Schedule pinned!");
                            }}
                ...
                <tr>
                  <td colSpan={10} className="px-4 py-8 text-center text-muted-foreground font-medium bg-muted/10">
                    {language === "de"
      ...
      <Dialog open={!!editingDay} onOpenChange={(open) => !open && setEditingDay(null)}>
        <DialogContent className="sm:max-w-[425px] font-sans">
          ...
          <div className="grid gap-4 py-4 text-xs">
            ...
            <div className="flex items-center justify-between rounded-lg border border-border/80 p-3 bg-muted/10 shadow-sm">
              <div className="space-y-0.5">
                <Label htmlFor="dialog-holiday" className="text-xs font-bold text-foreground cursor-pointer">
            ...
            {/* Setter Capacity */}
            <div className={cn("space-y-2 transition-all duration-200", inputIsHoliday && "opacity-40 pointer-events-none")}>
            ...
            <div className={cn("space-y-2 transition-all duration-200", inputIsHoliday && "opacity-40 pointer-events-none")}>
              <div className="flex justify-between items-center">
                <Label htmlFor="dialog-operator-slider" className="text-xs font-bold text-foreground">
          ...
          <DialogFooter className="gap-2 sm:gap-0">
            ...
            <Button variant="default" size="sm" onClick={handleSaveDayConfig} className="bg-primary text-primary-foreground hover:bg-primary/90">
