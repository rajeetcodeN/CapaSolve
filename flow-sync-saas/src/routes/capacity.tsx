...
export const Route = createFileRoute("/capacity")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Dedicated daily capacity settings and resource summary dashboard." },
    ],
  }),
...
function CapacityPage() {
  ...
  const getMachineColor = (idx: number, isSetup = false) => {
    const opColors = ["#10b981", "#3b82f6", "#f59e0b", "#ec4899", "#8b5cf6", "#14b8a6", "#f97316", "#06b6d4"];
    const setColors = ["#8b5cf6", "#6366f1", "#a855f7", "#db2777", "#ec4899", "#3b82f6", "#d97706", "#0284c7"];
  ...
  const [visiblePanels, setVisiblePanels] = useState<string[]>(["settings", "gantt", "pivot", "chart", "sop"]);
  ...
  const [dragOverCell, setDragOverCell] = useState<{ machineId: string; hour: number } | null>(null);
  ...
  const calendarDays = useMemo(() => {
    ...
    const list: { dayStr: string; dayNum: number; isPadding: boolean; hasSchedules: boolean; hasCustomCap: boolean; isHoliday?: boolean }[] = [];

    // Padding for previous month
    for (let i = 0; i < offset; i++) {
      list.push({ dayStr: "", dayNum: 0, isPadding: true, hasSchedules: false, hasCustomCap: false });
    }
    ...
    for (let hIdx = 0; hIdx < WORKING_HOURS_PER_DAY; hIdx++) {
      ...
      slots.forEach((s) => {
        if (s.date === selectedDateStr && s.hourStart === hr) {
          ...
          if (s.slotType === "R") {
            setterMinutesByMachine[s.machineId] = (setterMinutesByMachine[s.machineId] || 0) + s.minutesUsed;
            ...
            const pct = typeof s.manpowerPct === "number" && !isNaN(s.manpowerPct) ? s.manpowerPct : 0;
            const opMins = s.minutesUsed * pct;
            operatorMinutesByMachine[s.machineId] = (operatorMinutesByMachine[s.machineId] || 0) + opMins;
    ...
    if (active && payload && payload.length) {
      ...
      return (
        <div className="bg-background border border-border p-3 rounded-md shadow-md text-xs space-y-2 max-w-sm">
          <p className="font-extrabold text-foreground border-b border-border/60 pb-1">{label} Uhr</p>
          <p className="font-semibold text-foreground">
            Total Load: <span className="font-mono text-primary font-extrabold">{totalLoad.toFixed(1)}%</span> (Limit: {limit}%)
          </p>
          {breakdown.length > 0 ? (
            <div className="space-y-1.5 border-t border-border/40 pt-1.5">
              <p className="text-[10px] text-muted-foreground uppercase font-bold">Process Breakdown:</p>
              <div className="max-h-[150px] overflow-y-auto space-y-2 pr-1">
                {breakdown.map((item: any, idx: number) => (
                  <div key={idx} className="flex flex-col border-b border-border/10 pb-1.5 last:border-0 last:pb-0 last:border-none">
                    <div className="flex justify-between items-center gap-4 text-foreground font-semibold">
                      <span>Order {item.orderCode} (Step {item.stepId})</span>
                      <span className="font-mono text-primary font-bold">{item.pct.toFixed(1)}%</span>
                    </div>
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      {/* 1. Header with page description */}
      <div className="flex flex-wrap items-end justify-between gap-4 border-b border-border/60 pb-5">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">
            {language === "de" ? "Kapazitäts- & Ressourcenplaner" : "Daily Capacity & Resource Planner"}
          </h1>
          <p className="text-muted-foreground text-sm">
            {language === "de"
              ? "Konfigurieren Sie individuelle Ressourcenobergrenzen für bestimmte Tage, optimieren Sie Auslastungen und prüfen Sie stundengenaue Belegungen, Pivot-Tabellen und Auslastungsdiagramme."
        ...
        <div className="flex flex-wrap items-center gap-3 bg-muted/40 border border-border/50 px-4 py-2 rounded-lg text-xs font-semibold shadow-inner">
          <span className="text-muted-foreground flex items-center gap-1 uppercase tracking-wider">
            <Clock className="h-3.5 w-3.5" /> {language === "de" ? "Ansichten umschalten:" : "Toggle Panels:"}
          </span>
          <label className="flex items-center gap-1.5 cursor-pointer hover:text-primary transition-all">
            <Checkbox checked={visiblePanels.includes("settings")} onCheckedChange={() => togglePanel("settings")} />
            {language === "de" ? "Einstellungen" : "Settings"}
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer hover:text-primary transition-all">
            <Checkbox checked={visiblePanels.includes("gantt")} onCheckedChange={() => togglePanel("gantt")} />
            {language === "de" ? "Gantt-Diagramm" : "Gantt Chart"}
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer hover:text-primary transition-all">
            <Checkbox checked={visiblePanels.includes("pivot")} onCheckedChange={() => togglePanel("pivot")} />
            {language === "de" ? "Pivot-Tabelle" : "Pivot Table"}
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer hover:text-primary transition-all">
            <Checkbox checked={visiblePanels.includes("chart")} onCheckedChange={() => togglePanel("chart")} />
            {language === "de" ? "Auslastungsdiagramm" : "Composed Chart"}
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer hover:text-primary transition-all">
            <Checkbox checked={visiblePanels.includes("sop")} onCheckedChange={() => togglePanel("sop")} />
            {language === "de" ? "SOP-Meldungen" : "SOP Panel"}
          </label>
          <label className="flex items-center gap-1.5 cursor-pointer hover:text-primary transition-all">
            <Checkbox checked={visiblePanels.includes("warnings")} onCheckedChange={() => togglePanel("warnings")} />
      ...
      <div className="grid gap-6 md:grid-cols-3">
                ...
                return (
                  <button
                    ...
                    className={cn(
                      ...
                      isSelected ? "shadow-md font-extrabold scale-105" : (
                        cd.isHoliday ? "bg-rose-500/15 text-rose-600 dark:text-rose-400 border border-rose-500/30" : (
                          cd.hasSchedules ? "bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 border border-emerald-500/20" : "text-muted-foreground"
                        )
                      )
                    )}
                    style={isSelected ? { backgroundColor: "#0f172a", color: "#f8fafc" } : undefined}
                  >
                    <span>{cd.dayNum}</span>
                    {cd.isHoliday ? (
                      <span className={cn(
        ...
        {visiblePanels.includes("settings") ? (
          <Card className="border border-border/80 shadow-md bg-card flex flex-col justify-between">
            ...
            <CardContent className="space-y-4 pt-4 flex-1">
              ...
              <div className="flex items-center justify-between pb-3 border-b border-border/40">
                <Label htmlFor="holiday-toggle" className="font-bold text-foreground flex flex-col gap-0.5 cursor-pointer">
                ...
                <input
                  ...
                  onChange={(e) => {
                    ...
                    toast.success(
                      isHoliday
                        ? (language === "de" ? "Tag als Feiertag markiert!" : "Day marked as Holiday!")
                        : (language === "de" ? "Feiertag entfernt!" : "Holiday removed!")
                    );
                  }}
              ...
              <div className={cn("space-y-1.5 transition-opacity duration-200", dailyCapacities?.[selectedDateStr]?.isHoliday ? "opacity-40" : "opacity-100")}>
                <div className="flex justify-between items-center text-xs">
                  <Label htmlFor="setter-limit-in" className="font-bold text-muted-foreground uppercase">
                ...
                <div className="flex items-center gap-1.5">
                  <Input
                    ...
                    onChange={(e) => setDailyCapacity(selectedDateStr, { setter: Math.max(0, parseInt(e.target.value) || 0) })}
              ...
              <div className={cn("space-y-1.5 transition-opacity duration-200", dailyCapacities?.[selectedDateStr]?.isHoliday ? "opacity-40" : "opacity-100")}>
                <div className="flex justify-between items-center text-xs">
                  <Label htmlFor="operator-limit-in" className="font-bold text-muted-foreground uppercase">
                ...
                <div className="flex items-center gap-1.5">
                  <Input
                    ...
                    disabled={!!dailyCapacities?.[selectedDateStr]?.isHoliday}
                    onChange={(e) => setDailyCapacity(selectedDateStr, { process: Math.max(0, parseInt(e.target.value) || 0) })}
                    className="h-8 text-xs font-mono w-[90px]"
                  />
                  <span className="text-xs font-bold text-muted-foreground font-mono">%</span>
                </div>
          ...
          <div className="flex items-center justify-center border border-dashed rounded-lg bg-muted/10 text-muted-foreground text-xs font-medium">
            {language === "de" ? "Einstellungen-Panel ist ausgeblendet. Aktivieren Sie 'Einstellungen' oben." : "Settings panel is hidden. Check 'Settings' above."}
          </div>
        )}
        ...
        <Card className="border border-border/80 shadow-md bg-card">
          ...
          <CardContent className="space-y-4 pt-4 text-xs">
            <div className="space-y-1">
              <div className="flex justify-between items-baseline">
                <span className="text-muted-foreground font-bold uppercase tracking-wider text-[10px]">
                  {language === "de" ? "Einrichter (Rüsten R) Tagesauslastung" : "Setter (Setup R) Daily Utilization"}
                </span>
                <span className={cn("font-bold text-sm font-mono", dayActualMinutes.setterUtil > 100 ? "text-red-500 animate-pulse" : "text-sky-600")}>
              ...
              <div className="w-full h-2 rounded bg-slate-100 dark:bg-slate-800 overflow-hidden flex border border-border/10">
                <div 
                  ...
                  className={cn("h-full", dayActualMinutes.setterUtil > 100 ? "bg-red-500" : "bg-sky-500")}
                />
              </div>
            ...
            <div className="space-y-1">
              <div className="flex justify-between items-baseline">
                <span className="text-muted-foreground font-bold uppercase tracking-wider text-[10px]">
                  {language === "de" ? "Bediener (Maschine M) Tagesauslastung" : "Operator (Machining M) Daily Utilization"}
                </span>
                <span className={cn("font-bold text-sm font-mono", dayActualMinutes.operatorUtil > 100 ? "text-red-500 animate-pulse" : "text-emerald-600")}>
              ...
              <div className="w-full h-2 rounded bg-slate-100 dark:bg-slate-800 overflow-hidden flex border border-border/10">
                <div 
                  ...
                  className={cn("h-full", dayActualMinutes.operatorUtil > 100 ? "bg-red-500" : "bg-emerald-500")}
                />
              </div>
              <p className="text-[10px] text-muted-foreground font-mono">
                {language === "de"
                ...
                <div className="flex items-start gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      toast.success(e.target.checked ? "Process overlap enabled!" : "Process overlap disabled!");
                    }}
                    className="h-3.5 w-3.5 mt-0.5 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer"
                  />
                  <Label htmlFor="process-overlap-checkbox-capacity" className="text-[10px] font-bold text-foreground cursor-pointer flex flex-col gap-0.5">
                    <span>{language === "de" ? "Prozessüberlappung erlauben" : "Allow Process Overlap"}</span>
                ...
                <div className="flex items-start gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      toast.success(e.target.checked ? "SOP Override enabled!" : "SOP Override disabled!");
                    }}
                    className="h-3.5 w-3.5 mt-0.5 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer"
                  />
                  <Label htmlFor="sop-override-checkbox-capacity" className="text-[10px] font-bold text-foreground cursor-pointer flex flex-col gap-0.5">
                    <span>{language === "de" ? "SOP-Startdatum ignorieren" : "Allow SOP Override"}</span>
                    <span className="text-[9px] text-muted-foreground font-normal">
                      {language === "de"
                ...
                <div className="flex items-start gap-2">
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      toast.success(e.target.checked ? "Max Utilize enabled!" : "Max Utilize disabled!");
                    }}
                    className="h-3.5 w-3.5 mt-0.5 rounded border-gray-300 text-emerald-600 focus:ring-emerald-500 cursor-pointer"
                  />
                  <Label htmlFor="max-utilize-checkbox-capacity" className="text-[10px] font-bold text-foreground cursor-pointer flex flex-col gap-0.5">
                    <span>{language === "de" ? "Auslastung maximieren (Verschieben in Gruppe)" : "Max Utilize (Shift in Group)"}</span>
                    <span className="text-[9px] text-muted-foreground font-normal">
                      {language === "de"
      ...
      {visiblePanels.includes("warnings") && warnings.length > 0 && (
        <Card className="border-red-500/20 bg-red-500/5 p-4 text-xs shadow-sm">
          <div className="flex items-center gap-2 font-bold text-red-600 dark:text-red-400 border-b border-red-500/15 pb-2 mb-2">
            ...
            {language === "de" ? "Kapazitätsüberschreitungen und Überlastungen im gewählten Zeitraum:" : "Scheduling & Stacking Overloads detected in selected range:"}
          </div>
          <ul className="list-inside list-disc text-muted-foreground font-mono space-y-1 max-h-[150px] overflow-y-auto pr-2">
            {warnings.map((w, i) => (
              <li key={i} className="whitespace-pre-line leading-relaxed">{w}</li>
            ))}
          </ul>
        </Card>
      )}
      ...
      {visiblePanels.includes("gantt") && (
        <Card className="border border-border/80 shadow-md overflow-hidden bg-card">
          ...
          <CardContent className="p-0 overflow-x-auto">
            <div style={{ width: 220 + WORKING_HOURS_PER_DAY * HOUR_WIDTH }} className="flex flex-col relative select-none">
              ...
              <div className="flex border-b border-border bg-muted/40 sticky top-0 z-20">
                ...
                <div className="flex text-[9px] font-semibold text-muted-foreground">
                  <div className="border-r border-border/30 px-3 py-1 bg-sky-500/5 text-center font-bold" style={{ width: 7 * HOUR_WIDTH }}>
                    {language === "de" ? "Schicht 1 (06:00–13:00)" : "Shift 1 (06:00–13:00)"}
                  </div>
                  <div className="px-3 py-1 bg-orange-500/5 text-center font-bold" style={{ width: 7 * HOUR_WIDTH }}>
              ...
              {machines.map((m) => {
                ...
                const dayProcesses = processes.filter((p) => {
                  if (p.status !== "SCHEDULED" || p.machineId !== m.id || !p.scheduledStart || !p.scheduledEnd) {
                    return false;
                  }
                    ...
                    <div className="w-[220px] shrink-0 border-r border-border px-4 py-2.5 bg-background sticky left-0 z-20 flex items-center justify-between shadow-[2px_0_5px_rgba(0,0,0,0.02)]">
                      <div>
                        ...
                        <div className="text-[10px] text-muted-foreground uppercase font-semibold">
                          {language === "de" ? `Gruppe ${m.machineGroupId}` : `Group ${m.machineGroupId}`}
                        </div>
                      </div>
                    </div>

                    {/* Timeline background cell dropzones */}
                    <div className="relative flex items-center" style={{ width: WORKING_HOURS_PER_DAY * HOUR_WIDTH, height: 48 }}>
                      <div className="absolute inset-0 flex pointer-events-none">
                        {Array.from({ length: WORKING_HOURS_PER_DAY }).map((_, hIdx) => {
                          const hr = SHIFT_1_START + hIdx;
                          const isShift1 = hr < SHIFT_1_END;
                          const isOver = dragOverCell && dragOverCell.machineId === m.id && dragOverCell.hour === hr;
                          ...
                          return (
                            <div
                              ...
                              onDragOver={(e) => {
                                e.preventDefault();
                                if (!dragOverCell || dragOverCell.machineId !== m.id || dragOverCell.hour !== hr) {
                                  setDragOverCell({ machineId: m.id, hour: hr });
                                }
                              }}
                              onDragLeave={() => {
                                setDragOverCell((curr) => curr && curr.machineId === m.id && curr.hour === hr ? null : curr);
                              }}
                      ...
                      {dayProcesses.map((p) => {
                        ...
                        const renderStart = new Date(Math.max(startD.getTime(), windowStart.getTime()));
                        ...
                        if (setupEndD > renderStart) {
                          const visibleSetupEnd = new Date(Math.min(setupEndD.getTime(), renderEnd.getTime()));
                          const visibleSetupHours = getWorkingHoursBetween(renderStart, visibleSetupEnd);
        ...
        <Card className="border border-border/80 shadow-md overflow-hidden bg-card">
          <CardHeader className="border-b border-border/50 pb-3">
            <CardTitle className="text-base font-bold flex items-center gap-2 text-foreground">
              ...
              Day Workstation Load Pivot Grid ({new Date(selectedDateStr).toLocaleDateString("de-DE")})
            </CardTitle>
            <CardDescription>
              Shows hourly loads per machine for R (Setup min), M (Machining min), and P (Manpower util load) in the selected day.
            </CardDescription>
          </CardHeader>
              ...
              <table className="w-full text-xs border-collapse">
                <thead>
                  <tr className="bg-muted border-b border-border">
                    <th colSpan={3} className="px-4 py-3 border-r border-border text-left font-bold uppercase tracking-wider sticky left-0 bg-muted z-20 w-[260px] min-w-[260px] max-w-[260px]">
                      Workstations / Hours
                    </th>
                    {Array.from({ length: WORKING_HOURS_PER_DAY }).map((_, hIdx) => {
                      const hr = SHIFT_1_START + hIdx;
                      return (
                        <th key={hr} colSpan={3} className="px-2 py-2 border-r border-border text-center font-bold text-foreground min-w-[100px] font-mono">
                          {String(hr).padStart(2, "0")}:00
                        </th>
                      );
                    })}
                  ...
                  <tr className="bg-muted border-b border-border font-medium text-muted-foreground">
                    <th className="px-4 py-1.5 text-left border-r border-border sticky left-0 bg-muted z-20 w-[90px] min-w-[90px] max-w-[90px]">Group</th>
                    <th className="px-2 py-1.5 text-left border-r border-border sticky left-[90px] bg-muted z-20 w-[90px] min-w-[90px] max-w-[90px]">Machine</th>
                    <th className="px-2 py-1.5 text-left border-r border-border sticky left-[180px] bg-muted z-20 w-[80px] min-w-[80px] max-w-[80px]">Auftrag</th>
                    
                    {Array.from({ length: WORKING_HOURS_PER_DAY }).map((_, hIdx) => (
                      <Fragment key={hIdx}>
                        <th className="px-0.5 py-1 text-center font-mono border-r border-border/20 text-sky-600 bg-sky-50/25 w-[30px] min-w-[30px]">R</th>
                        <th className="px-0.5 py-1 text-center font-mono border-r border-border/20 text-emerald-600 bg-emerald-50/25 w-[30px] min-w-[30px]">M</th>
                        <th className="px-0.5 py-1 text-center font-mono border-r border-border text-orange-600 bg-orange-50/25 w-[40px] min-w-[40px]">P</th>
                      </Fragment>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <Fragment key={g.id}>
                        ...
                        <tr className="bg-muted/10 font-bold border-b border-border">
                          <td colSpan={3} className="px-4 py-2 border-r border-border text-primary sticky left-0 bg-background z-10 font-bold w-[260px] min-w-[260px] max-w-[260px]">
                            {g.name}
                          </td>
                          {Array.from({ length: WORKING_HOURS_PER_DAY }).map((_, hIdx) => (
                        ...
                        <tr className="bg-slate-100 dark:bg-slate-800 font-bold border-b border-border/80">
                          <td colSpan={3} className="px-4 py-2 border-r border-border sticky left-0 bg-slate-100 dark:bg-slate-800 z-10 font-bold text-foreground w-[260px]">
                            {g.name} Ergebnis
                          </td>
                          {Array.from({ length: WORKING_HOURS_PER_DAY }).map((_, hIdx) => {
                            const hr = SHIFT_1_START + hIdx;
                  ...
                  <tr className="bg-slate-200 dark:bg-slate-700 font-bold border-b border-border/90 text-sm">
                    <td colSpan={3} className="px-4 py-2.5 border-r border-border sticky left-0 bg-slate-200 dark:bg-slate-700 z-10 font-bold text-primary w-[260px]">
                      Gesamtergebnis
                    </td>
                    {Array.from({ length: WORKING_HOURS_PER_DAY }).map((_, hIdx) => {
                      const hr = SHIFT_1_START + hIdx;
          ...
          <CardHeader className="flex flex-row items-center justify-between pb-4 flex-wrap gap-4 border-b border-border/40 bg-muted/10">
            <div>
              <CardTitle className="text-base font-bold flex items-center gap-2 text-foreground">
                ...
                SOP Start Orders ({new Date(selectedDateStr).toLocaleDateString("de-DE")})
              </CardTitle>
              <CardDescription>
                Manufacturing orders and their associated process steps scheduled to start production on this date.
              </CardDescription>
            </div>
          ...
          <CardContent className="pt-6">
            {sopOrdersForSelectedDay.length === 0 ? (
              <div className="text-center py-10 text-muted-foreground text-sm">
                Keine Aufträge haben an diesem Tag SOP Start.
              </div>
            ) : (
              <div className="space-y-6">
                {sopOrdersForSelectedDay.map((order) => {
                  ...
                  return (
                    <div key={order.id} className="border border-border/80 rounded-lg overflow-hidden bg-muted/5">
                      <div className="bg-muted/30 px-4 py-3 border-b border-border/60 flex flex-wrap items-center justify-between gap-4">
                        <div>
                          <span className="font-extrabold text-sm text-foreground">Auftrag: {order.orderId}</span>
                          <span className="ml-3 font-mono text-xs text-muted-foreground">Material: {order.material}</span>
                        </div>
                        <div className="flex items-center gap-3 text-xs">
                          <span className="text-muted-foreground font-semibold">Qty: <span className="font-mono text-foreground font-bold">{order.orderQty}</span></span>
                          <span className="text-muted-foreground font-semibold">SOP Time: <span className="font-mono text-foreground font-bold">{order.sopStartTime}</span></span>
                        </div>
                      </div>
                        ...
                        <table className="w-full text-left border-collapse text-xs">
                          <thead>
                            <tr className="border-b border-border bg-muted/10 text-muted-foreground uppercase font-bold text-[10px] tracking-wider">
                              ...
                              <th className="px-4 py-2">Maschine</th>
                              ...
                              <th className="px-4 py-2">Geplanter Zeitraum</th>
                            </tr>
                          </thead>
                          <tbody>
                            {orderProcs.map((proc) => {
                              ...
                              return (
                                <tr key={proc.id} className="border-b border-border/40 last:border-0 hover:bg-muted/10 transition-colors">
                                  <td className="px-4 py-2.5 font-mono font-bold text-foreground">{proc.processId}</td>
                                  <td className="px-4 py-2.5 font-mono text-muted-foreground">{proc.machineId}</td>
                                  <td className="px-4 py-2.5 text-foreground max-w-[200px] truncate" title={proc.processText}>{proc.processText}</td>
                                  <td className="px-4 py-2.5 text-center font-mono text-muted-foreground">{proc.setupTimeMin}m</td>
                                  <td className="px-4 py-2.5 text-center font-mono text-muted-foreground">{Math.round(proc.sumV2)}m</td>
                                  ...
                                  <td className="px-4 py-2.5 font-mono text-xs">
                                    {isScheduled && proc.scheduledStart && proc.scheduledEnd ? (
                                      <span className="text-foreground font-semibold">
                                        {new Date(proc.scheduledStart).toLocaleDateString("de-DE", { day: "2-digit", month: "short" })}: {new Date(proc.scheduledStart).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })} - {new Date(proc.scheduledEnd).toLocaleTimeString("de-DE", { hour: "2-digit", minute: "2-digit" })}
                                      </span>
                                    ) : (
                                      <span className="text-muted-foreground italic">Pending scheduling</span>
                                    )}
                                  </td>
                                </tr>
                              );
        ...
        <Card className="border border-border/80 shadow-md bg-card overflow-hidden">
          <CardHeader className="flex flex-row items-center justify-between pb-4 flex-wrap gap-4">
            <div>
              <CardTitle className="text-base font-bold flex items-center gap-2 text-foreground">
                <BarChart3 className="h-5 w-5 text-primary" />
                {language === "de" ? "Stündliche Auslastung vs. Kapazitätsgrenze" : "Hourly Load vs Capacity Limit"} ({new Date(selectedDateStr).toLocaleDateString(language === "de" ? "de-DE" : "en-US")})
              </CardTitle>
              <CardDescription>
                {language === "de" 
            ...
            <div className="flex flex-wrap items-center gap-3">
              ...
              <div className="flex items-center gap-1.5 text-xs">
                <span className="text-muted-foreground font-medium">{language === "de" ? "Maschine:" : "Workstation:"}</span>
                <Select value={chartMachineFilter} onValueChange={setChartMachineFilter}>
                  <SelectTrigger className="h-8 w-[160px] text-xs bg-background">
                    <SelectValue placeholder={language === "de" ? "Alle Maschinen" : "All Workstations"} />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    {machines.map((m) => (
                      <SelectItem key={`m-${m.id}`} value={m.id}>
                        {language === "de" ? `Maschine ${m.name || m.id}` : `Machine ${m.name || m.id}`}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
              ...
              <button
                ...
                title={language === "de" ? "Kapazitätsgrenzen-Linie ein/ausblenden" : "Toggle limit line"}
              >
              ...
              <div className="flex bg-muted p-1 rounded-md border border-border text-xs shrink-0 font-medium">
                <button
                  ...
                  className={cn(
                    ...
                    chartRole === "operator" ? "bg-background font-bold text-foreground shadow-sm" : "text-muted-foreground"
                  )}
                >
                ...
                <button
                  ...
                  className={cn(
                    "px-3 py-1 rounded transition-all cursor-pointer",
                    chartRole === "setter" ? "bg-background font-bold text-foreground shadow-sm" : "text-muted-foreground"
                  )}
                >
          ...
          <CardContent className="h-[300px]">
            <ResponsiveContainer width="100%" height={260}>
              <ComposedChart data={hourlyChartData} margin={{ top: 10, right: 10, bottom: 5, left: 10 }}>
                <CartesianGrid strokeDasharray="3 3" className="stroke-border/40" />
                <XAxis dataKey="hourLabel" className="text-[10px] font-mono text-muted-foreground" />
                <YAxis 
                  label={{ 
                ...
                {chartRole === "operator" && filteredChartMachines.map((m, idx) => (
                  <Bar 
                    key={`op-bar-${m.id}`}
                ...
                {chartRole === "setter" && filteredChartMachines.map((m, idx) => (
                  <Bar 
                    key={`set-bar-${m.id}`}
