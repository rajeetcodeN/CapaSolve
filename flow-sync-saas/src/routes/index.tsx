...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Optimize workstation timelines, reduce setup bottlenecks, and maximize OEE with AI-powered constraint scheduling." },
    ],
  }),
...
function LandingPage() {
  ...
  useEffect(() => {
    if (typeof window !== "undefined" && (window.location.hash === "#features" || window.location.href.includes("features"))) {
  ...
  const copy = {
    en: {
      ...
      heroSubtitle: "Constraint-based resource planning designed for modern factories. Reduce setup bottlenecks, allocate technicians, and sync timelines in real-time.",
      ...
      featuresSubtitle: "CapaSolve automates the complex constraints of machine capacity, setup technician loading, and order sequences.",
      featGantt: "Visual Gantt Scheduler",
      featGanttDesc: "Drag-and-drop process blocks across machines. Live collision alerts prevent double-booking.",
      featOEE: "Performance & OEE Analytics",
      featOEEDesc: "Track setup vs machining times. Identify workstation bottlenecks before they stall production.",
      ...
      featCloudDesc: "Safeguard scheduling states. Save, load, and share production plans across workstations.",
      featAccess: "Role-Based Access Control",
      featAccessDesc: "Segregate accounts. Developers modify plans, admins manage billing/team, and guests view read-only charts.",
      featExcel: "Instant CSV/Excel Processing",
      featExcelDesc: "Directly import standard process sheets. Confirmed formulas compute durations instantly.",
      roiTitle: "Calculate Your ROI with CapaSolve",
      roiSubtitle: "Input your factory size to estimate potential time savings and annual financial impact.",
      ...
      roiDisclaimer: "Estimates are based on average customer metrics: $50/hour internal overhead rate and 30% planning efficiency gains.",
      ctaTitle: "Ready to eliminate scheduling bottlenecks?",
      ctaSubtitle: "Create a free trial account, select your role (Admin or Developer), and begin optimizing your timelines in under 5 minutes.",
    }
  };
        ...
        <div className="mx-auto max-w-[1400px] grid md:grid-cols-2 gap-12 items-center relative z-10">
          ...
          <div className="space-y-6 text-left">
            ...
            <div className="flex flex-wrap gap-4 pt-2">
              <Button asChild size="lg" className="cursor-pointer font-bold shadow-lg shadow-primary/25 bg-primary text-primary-foreground hover:bg-primary/95 transition-all hover:scale-[1.02]">
                <Link to="/signup">{str.startFree} <ArrowRight className="ml-2 h-4 w-4" /></Link>
              </Button>
              <Button asChild size="lg" variant="outline" className="cursor-pointer font-bold border-border/80 hover:bg-accent">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex-grow grid grid-cols-6 gap-1 h-7">
                    <div className="col-span-2 bg-blue-500/20 border border-blue-500/50 rounded flex items-center justify-center text-[9px] text-blue-600 dark:text-blue-400 font-bold shadow-xs">Setup Rüst (R)</div>
                    <div className="col-span-3 bg-emerald-500/20 border border-emerald-500/50 rounded flex items-center justify-center text-[9px] text-emerald-600 dark:text-emerald-400 font-bold shadow-xs">Milling (M)</div>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex-grow grid grid-cols-6 gap-1 h-7">
                    <div className="col-span-1 border border-dashed border-border/80 rounded" />
                    <div className="col-span-2 bg-blue-500/20 border border-blue-500/50 rounded flex items-center justify-center text-[9px] text-blue-600 dark:text-blue-400 font-bold shadow-xs">Setup Rüst (R)</div>
                    <div className="col-span-3 bg-emerald-500/20 border border-emerald-500/50 rounded flex items-center justify-center text-[9px] text-emerald-600 dark:text-emerald-400 font-bold shadow-xs">Assembly (M)</div>
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between gap-3">
                  ...
                  <div className="flex-grow grid grid-cols-6 gap-1 h-7">
                    <div className="col-span-3 bg-emerald-500/20 border border-emerald-500/50 rounded flex items-center justify-center text-[9px] text-emerald-600 dark:text-emerald-400 font-bold shadow-xs">Drilling (M)</div>
                    <div className="col-span-2 bg-blue-500/20 border border-blue-500/50 rounded flex items-center justify-center text-[9px] text-blue-600 dark:text-blue-400 font-bold shadow-xs">Setup Rüst (R)</div>
                    <div className="col-span-1 border border-dashed border-border/80 rounded" />
                  </div>
                </div>
      ...
      <section className="py-12 px-6 border-b border-border/40 bg-muted/30">
        <div className="mx-auto max-w-[1400px] grid grid-cols-1 sm:grid-cols-3 gap-8 text-center">
          <div className="space-y-1">
            <div className="text-3xl md:text-5xl font-extrabold text-primary tracking-tight font-mono">+{oeeImprovement}%</div>
            <div className="text-xs uppercase font-bold tracking-wider text-muted-foreground">{str.metricOEE}</div>
          </div>
          <div className="space-y-1">
            <div className="text-3xl md:text-5xl font-extrabold text-primary tracking-tight font-mono">{weeklyHoursSaved} hrs</div>
            <div className="text-xs uppercase font-bold tracking-wider text-muted-foreground">{str.metricHours}</div>
          </div>
          <div className="space-y-1">
            <div className="text-3xl md:text-5xl font-extrabold text-primary tracking-tight font-mono">&lt; 3 Months</div>
            <div className="text-xs uppercase font-bold tracking-wider text-muted-foreground">{str.metricROI}</div>
          </div>
        </div>
      </section>
      ...
      <section className="py-24 px-6 border-b border-border/40 bg-muted/20">
        <div className="mx-auto max-w-[1100px] text-center space-y-12">
          <div className="space-y-4 max-w-2xl mx-auto">
            <h2 className="text-3xl font-extrabold tracking-tight text-foreground">{str.roiTitle}</h2>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center bg-card border border-border/80 p-8 rounded-2xl shadow-xl text-left relative overflow-hidden">
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-foreground uppercase tracking-wider">{str.roiMachines}</label>
                  <span className="text-sm font-extrabold text-primary font-mono">{machines} Machines</span>
                </div>
                <input
                  type="range"
                  min="2"
              ...
              <div className="space-y-3">
                <div className="flex justify-between items-center">
                  <label className="text-xs font-bold text-foreground uppercase tracking-wider">{str.roiOperators}</label>
                  <span className="text-sm font-extrabold text-primary font-mono">{operators} Operators</span>
                </div>
                <input
                  type="range"
                  min="1"
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 bg-muted/40 p-6 rounded-xl border border-border/60">
              <div className="space-y-1">
                <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">{str.roiSavedHours}</span>
                <div className="text-2xl font-extrabold text-foreground font-mono">~{weeklyHoursSaved} hrs / wk</div>
              </div>
              <div className="space-y-1">
                <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">{str.roiOEE}</span>
                <div className="text-2xl font-extrabold text-emerald-600 dark:text-emerald-400 font-mono">+{oeeImprovement}%</div>
              </div>
              <div className="col-span-1 sm:col-span-2 space-y-1 pt-4 border-t border-border/40">
                <span className="text-[10px] text-muted-foreground uppercase font-bold tracking-wider">{str.roiSavedMoney}</span>
                <div className="text-3xl font-extrabold text-primary tracking-tight font-mono">
                  {language === "de" ? `~${annualSavings.toLocaleString()} €` : `~$${annualSavings.toLocaleString()}`} / yr
                </div>
              </div>
            </div>
          </div>
      ...
      <section className="py-24 px-6 bg-primary text-primary-foreground text-center relative overflow-hidden">
        ...
        <div className="mx-auto max-w-3xl space-y-6 relative z-10">
          ...
          <div className="flex justify-center gap-4 pt-4">
            <Button asChild size="lg" variant="secondary" className="cursor-pointer font-bold shadow-lg bg-background text-foreground hover:bg-background/90">
              <Link to="/signup">{str.startFree}</Link>
            </Button>
            <Button asChild size="lg" variant="outline" className="cursor-pointer font-bold border-primary-foreground/40 bg-transparent hover:bg-white/10 hover:text-white text-primary-foreground">
              <Link to="/dashboard">{str.viewDemo}</Link>
            </Button>
          </div>
        </div>
